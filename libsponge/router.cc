...
void Router::route_one_datagram(InternetDatagram &dgram) {
    ...
    int match_id = -1;
    int longest_prefix_length = -1; // 初始化为不存在的最小值

    uint32_t ip = dgram.header().dst;

    for (size_t i = 0; i < _route_table.size(); i++) {
        auto entry = _route_table[i];
        if (!entry.prefix_length || (ip ^ entry.route_prefix) >> (32 - entry.prefix_length) == 0) {
    ...
    dgram.header().ttl -= 1;
    auto next_hop = _route_table[match_id].next_hop;
    auto interface_num = _route_table[match_id].interface_num;

    if (next_hop.has_value()) {
        interface(interface_num).send_datagram(dgram, next_hop.value());
    } else {
        interface(interface_num).send_datagram(dgram, Address::from_ipv4_numeric(ip));
    }
}
