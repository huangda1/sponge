...
class NetworkInterface {
  private:
    ...
    // std::pair<EthernetAddress, size_t> can be replaced by arpEntry
    struct ArpEntry {
        size_t caching_time;
    ...
    struct WaitingIpEntry {
        InternetDatagram dgram;
        Address next_hop;
        size_t waiting_time;
    };

    // ip datagram queue -> ip, nexthop, t
    std::vector<WaitingIpEntry> _WaitingList{};
    std::map<uint32_t, size_t> _Waiting_time{};

    // start from 0 to avoid size_t is negtive problem
    const size_t _max_arp_entry_ttl = 30 * 1000;
    const size_t _max_arp_response_ttl = 5 * 1000;
